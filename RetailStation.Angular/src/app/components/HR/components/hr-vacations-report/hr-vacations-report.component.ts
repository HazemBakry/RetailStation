...
@Component({
  selector: 'app-hr-vacations-report',
  templateUrl: './hr-vacations-report.component.html',
  styleUrls: ['./hr-vacations-report.component.css']
})
export class HrVacationsReportComponent implements OnInit {
  TitleList = ['لموارد البشرية', 'تقارير ألأجازات'];
  showLoader: boolean;
  ...
  onSearch() {
    ...
    this.getAllEmployeeVacationsData();
  }
  getAllEmployeeVacationsData() {
    this.mapFilters();
    this.showLoader = true;
    this.hrService.GetAllEmployeeVacationsData(this.pagedResponseModel).subscribe(data => {
  ...
  exportData() {
    ...
    this.hrService.GetEmployeeVacation_Export(this.pagedResponseModel).subscribe(data => {
      if (data.isSuccess) {
