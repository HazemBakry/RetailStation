...
export class ItemsComponent implements OnInit {
  ...
  loadData() {
    ...
    this.salesService.GetItemsData(this.pagedResponseModel).subscribe(data => {
  ...
  exportData(categoryId: number = 0) {
    ...
    this.salesService.ExportItems(this.pagedResponseModel, categoryId).subscribe((data: ActionsResponseModel) => {
      if (data.isSuccess) {
  ...
  addNewItem() {
    ...
    this.salesService.AddNewItem(this.formData).subscribe((data: ActionsResponseModel) => {
      if (data?.isSuccess) {
  ...
  editItem() {
    this.showAddLoader = true;
    this.salesService.EditItem(this.itemModel.itemId, this.formData).subscribe((data: ActionsResponseModel) => {
      if (data?.isSuccess) {
        this.formGroup?.reset();
        // this.initNewForm();
        this.toaster.success(data?.message);
  ...
  changeItemStatus(ItemId: number) {
    this.salesService.ChangeItemActiveStatus(ItemId).subscribe(data => {
      if (data.isSuccess) {
        this.toaster.success(data.message);
        this.loadData();
      } else {
  ...
  deleteItem() {
    this.showAddLoader = true;
    this.salesService.DeleteItem(this.selectedItemId).subscribe(data => {
  ...
  getItemSuppliersByItemId(itemId: number) {
    this.itemSuppliers = [];
    this.salesService.GetSuppliersByItemId(itemId).subscribe(data => {
