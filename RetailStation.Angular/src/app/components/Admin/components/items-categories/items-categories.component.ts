...
export class ItemsCategoriesComponent implements OnInit {
  ...
  pagedResponseModel: PagedResponseDTO<ItemCategoryModel[]> = {
    results: [],
    filterList: [],
    pageSize: 10,
    currentPage: 1,
  ...
  saveCategoriesDisplayOrder() {
    this.pagedResponseModel.results.forEach((el, i) => el.displayOrder = i + 1)
    const SortedItems: CategorySortModel[] = [...this.pagedResponseModel.results.map(x => {
