...
export class ItemsCategoriesComponent implements OnInit {
  TitleList = ['التشغيل', 'مجموعات الأصناف'];
  itemCategories: any[] = [];
  ...
  getItemCategories() {
    ...
    this.operationService.GetItemCategories().subscribe((data: PagedResponseDTO<ItemCategoryModel[]>) => {
  ...
  changeCategoryStatus(CategoryId: any) {
    this.operationService.ChangeItemCategoryActiveStatus(CategoryId).subscribe(data => {
      if (data.isSuccess) {
  ...
  addNewCategory() {
    ...
    this.operationService.AddNewItemCategory(this.categoryModel).subscribe((data: ActionsResponseModel) => {
      if (data?.isSuccess) {
        this.formGroup?.reset();
        this.toaster.success(data?.message);
  ...
  editCategory() {
    ...
    this.showAddLoader = true;
    this.operationService.EditItemCategory(this.categoryModel.itemCategoryId, this.categoryModel).subscribe((data: ActionsResponseModel) => {
      if (data?.isSuccess) {
        this.formGroup?.reset();
        // this.initNewForm();
        this.toaster.success(data?.message);
  ...
  submitDeleteAction() {
    this.operationService.DeleteItemCategory(this.selectedItemCategoryId).subscribe(data => {
      if (data?.isSuccess) {
        this.modalService?.dismissAll();
        this.getItemCategories();
        this.toaster.success(data?.message);
  ...
  changeCategoriesDisplayOrder(SortedItems: CategorySortModel[]) {
    ...
    this.showAddLoader = true;
    this.operationService.ChangeCategoriesDisplayOrder(SortedItems).subscribe(data => {
      if (data.isSuccess) {
        this.toaster.success(data.message);
        this.getItemCategories();
      } else {
