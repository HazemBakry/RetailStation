import { Injectable } from '@angular/core';
import * as pdfMake from 'pdfmake/build/pdfmake';
import * as pdfFonts from 'pdfmake/build/vfs_fonts';

(<any>pdfMake).vfs = pdfFonts.pdfMake.vfs;

import { TDocumentDefinitions, TFontDictionary } from 'pdfmake/interfaces';
import { environment } from 'src/environments/environment';
import { JournalEntryModel } from '../../GeneralAccounts/models/GeneralAccounts/JurnalEntryModel';

@Injectable({
  providedIn: 'root',
})


export class PDFExportService {
  systemUrl = environment.systemUrl;
  constructor() {}

  generatePDF(entryModel:JournalEntryModel,action = 'open') {

    
    const pdfFonts = {
      CustomArabic: {
        normal: this.systemUrl + 'assets/fonts/PDFMakeArabic.ttf',
        bold: this.systemUrl + 'assets/fonts/PDFMakeArabic.ttf',
        italics: this.systemUrl + 'assets/fonts/PDFMakeArabic.ttf',
        bolditalics: this.systemUrl + 'assets/fonts/PDFMakeArabic.ttf',
      },
    };

    let docDefinition: TDocumentDefinitions = {
      content: [
        {
          columns: [
            [
              {
                // this.systemUrl+
                // image: '../../../../assets/images/erp-logo.jpg',
                image:
                  'data:image/jpeg;base64,',
                width: 80,
                height: 80,
                // alignment: 'right',
              },
            ],
            [
              {
                // text: 'شركة مطاعم مشوار',
                text: 'Restaurant Company',
                bold: true,
                alignment: 'right',
                marginTop: 25,
              },
              // { text: this.invoice.email },
              // { text: this.invoice.contactNo },
            ],

            // [
            //   {
            //     text: `Date: ${new Date().toLocaleString()}`,
            //     alignment: 'right',
            //   },
            //   {
            //     text: `Bill No : ${(Math.random() * 1000).toFixed(0)}`,
            //     alignment: 'right',
            //   },
            // ],
          ],
        },
        // {
        //   text: 'ELECTRONIC SHOP',
        //   fontSize: 16,
        //   alignment: 'center',
        //   color: '#047886',
        // },
        {
          text: '',
          margin: [0, 0, 0, 25],
        },
        {
          text: 'Daily Voucher',
          fontSize: 16,
          bold: true,
          alignment: 'center',
          decoration: 'underline',
          // color: 'skyblue',
        },
        {
          text: '',
          margin: [0, 0, 0, 25],
        },
        {
          columns: [
            [
              {
                text: `Date: ${entryModel?.docNumber?.toLocaleString()}`,
              },
              {
                text: `Doc Number : ${entryModel?.docNumber}`,
              },
            ],
            [
              {
                text: `Entry Number :  ${entryModel?.entryDate}`,
                alignment: 'right',
              },
              {
                text: `Entry Type : ${entryModel?.entryNumber}`,
                alignment: 'right',
              },
              {
                text: `Notes : ${entryModel?.entryNumber}`,
                alignment: 'right',
              },
            ],
          ],
        },
        {
          text: '',
          margin: [0, 0, 0, 25],
        },
        {
          table: {
            headerRows: 1,
            widths: ['*', '*', '*', '*', '*', '*'],
            body: [
              [
                { text: 'Account Number', bold: true ,fillColor:'gray'},
                { text: 'Account', bold: true ,fillColor:'gray'},
                { text: 'Deptor', bold: true ,fillColor:'gray'},
                { text: 'Creditor', bold: true ,fillColor:'gray'},
                { text: 'Cost Center', bold: true ,fillColor:'gray'},
                { text: 'Notes', bold: true ,fillColor:'gray'},
                
              ],
              ...entryModel?.journalEntryAccounts?.map((p) => [
                p.accountNumber,
                p.accountName,
                p.debit.toFixed(2),
                p.credit.toFixed(2),
                p.costCenterId,
                p.notes,
              ]),
              [

                { text: 'Total Amount', colSpan: 2 },
                {},
                {text:entryModel?.journalEntryAccounts
                  ?.reduce((sum, p) => sum + p.debit, 0)
                  .toFixed(2)},
                {text:entryModel?.journalEntryAccounts
                  ?.reduce((sum, p) => sum + p.credit, 0)
                  .toFixed(2)},
                {},
                {},
              ],
            ],
          },
        },
        // {
        //   text: 'Order Details',
        //   style: 'sectionHeader',
        // },
        //   {
        //     text: 'Additional Details',
        //     style: 'sectionHeader',
        //   },
        //   // {

        //   //     text: this.invoice.additionalDetails,
        //   //     margin: [0, 0 ,0, 15]
        //   // },
        //   // {
        //   //   columns: [
        //   //     [{ qr: `${this.invoice.customerName}`, fit: '50' }],
        //   //     [{ text: 'Signature', alignment: 'right', italics: true}],
        //   //   ]
        //   // },
        //   {
        //     text: 'Terms and Conditions',
        //     style: 'sectionHeader',
        //   },
        //   {
        //     ul: [
        //       'Order can be return in max 10 days.',
        //       'Warrenty of the product will be subject to the manufacturer terms and conditions.',
        //       'This is system generated invoice.',
        //     ],
        //   },
      ],
      styles: {
        sectionHeader: {
          bold: true,
          decoration: 'underline',
          fontSize: 14,
          margin: [0, 15, 0, 15],
          // font: 'Roboto',
        },
        header: {
          fontSize: 18,
          bold: true,
          alignment: 'right',
          margin: [0, 0, 0, 10],
          // font: 'Roboto',
        },
        body: {
          fontSize: 14,
          alignment: 'right',
          margin: [0, 0, 0, 10],
          // font: 'Roboto',
        },
      },
    };

    if (action === 'download') {
      pdfMake.createPdf(docDefinition).download();
    } else if (action === 'print') {
      pdfMake.createPdf(docDefinition).print();
    } else {
      pdfMake.createPdf(docDefinition).open();
    }
  }
}
